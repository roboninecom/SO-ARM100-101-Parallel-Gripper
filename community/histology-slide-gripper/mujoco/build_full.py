...
def sub(parent, tag, **kw):
    ...
    for k, v in kw.items(): e.set(k.rstrip("_"), v)
    return e

# options / visual quality
opt = ET.Element("option"); opt.set("timestep", "0.002"); opt.set("integrator", "implicitfast")
root.insert(1, opt)
...
inc = ET.Element("include"); inc.set("file", "gripper_assets.xml"); root.insert(4, inc)
...
inert.set("mass", "0.08"); inert.set("pos", "0 -0.01 0.005")
inert.set("diaginertia", "6e-05 6e-05 4e-05"); inert.set("quat", "1 0 0 0")

# hang the gripper off link5
link5 = wb.iter("body")
link5 = [b for b in wb.iter("body") if b.get("name") == "link5_1"][0]
mount = ET.SubElement(link5, "body")
mount.set("name", "sg_mount"); mount.set("pos", MOUNT_POS); mount.set("quat", MOUNT_QUAT)
...
for j, kp in (("base_link_to_link1", 12), ("link1_to_link2", 18), ("link2_to_link3", 14),
    ...
    e.set("name", j); e.set("joint", j); e.set("kp", str(kp)); e.set("kv", str(round(kp/6, 2)))
...
e = ET.SubElement(con, "exclude"); e.set("body1", "link5_1"); e.set("body2", "slide_gripper_base")
...
e = ET.SubElement(con, "exclude"); e.set("body1", "world"); e.set("body2", "link1_1")
