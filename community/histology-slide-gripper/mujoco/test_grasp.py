...
cam.lookat[:] = [0, -0.028, 0.06]; cam.distance = 0.40
cam.azimuth = 140; cam.elevation = -15
...
def run(n, ctrl, ramp=False):
    ...
    for k in range(n):
        ...
        for a, v in ctrl.items(): d.ctrl[a] = start[a] + (v - start[a]) * f
        mujoco.mj_step(m, d)

with mujoco.Renderer(m, 900, 1100) as r:
    run(400, {GRIP: 0.0, LIFT: 0.0});                 shot(r, "open")
    run(1200, {GRIP: 0.0355, LIFT: 0.0}, ramp=True);             shot(r, "clamped")   # over-close -> squeeze
    run(1500, {GRIP: 0.0355, LIFT: 0.15}, ramp=True); run(800, {GRIP: 0.0355, LIFT: 0.15});            shot(r, "lifted")
